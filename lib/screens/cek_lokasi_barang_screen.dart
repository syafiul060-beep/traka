...
/// Pengirim lihat: driver + penerima. Penerima lihat: driver + pengirim.
/// Info: lokasi driver, estimasi km ke penerima, estimasi waktu sampai.
...
class _CekLokasiBarangScreenState extends State<CekLokasiBarangScreen> {
  ...
  String _driverLocationText = 'Memuat lokasi...';
  ...
  Future<void> _updateDriverLocationText(double lat, double lng) async {
    try {
      ...
      if (mounted) {
        setState(() => _driverLocationText = 'Driver sedang dalam perjalanan');
      }
    }
  }
    ...
    return Scaffold(
      body: FutureBuilder<DocumentSnapshot<Map<String, dynamic>>>(
        ...
        builder: (context, orderSnap) {
          ...
          final receiverLat = order.receiverLat ?? order.destLat;
          final receiverLng = order.receiverLng ?? order.destLng;
          final passengerLat = order.passengerLat ?? order.originLat;
          final passengerLng = order.passengerLng ?? order.originLng;
          ...
          return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
            ...
            builder: (context, statusSnap) {
              ...
              final destLat = receiverLat;
              final destLng = receiverLng;
              double? distanceMeters;
              String distanceText = '-';
              String etaText = '-';
              if (destLat != null && destLng != null) {
                distanceMeters = Geolocator.distanceBetween(lat, lng, destLat, destLng);
                ...
                etaText = '${eta.hour.toString().padLeft(2, '0')}.${eta.minute.toString().padLeft(2, '0')}';
              }
              ...
              if (widget.isPengirim && receiverLat != null && receiverLng != null) {
                markers.add(Marker(
                  markerId: const MarkerId('penerima'),
                  position: LatLng(receiverLat, receiverLng),
                  icon: BitmapDescriptor.defaultMarkerWithHue(BitmapDescriptor.hueOrange),
                  infoWindow: InfoWindow(title: 'Penerima', snippet: order.receiverLocationText ?? order.destText),
                ));
              } else if (!widget.isPengirim && passengerLat != null && passengerLng != null) {
                markers.add(Marker(
                  markerId: const MarkerId('pengirim'),
                  position: LatLng(passengerLat, passengerLng),
                  icon: BitmapDescriptor.defaultMarkerWithHue(BitmapDescriptor.hueOrange),
                  infoWindow: InfoWindow(title: 'Pengirim', snippet: order.passengerLocationText ?? order.originText),
                ));
              }
                            ...
                            children: [
                              Row(
                                children: [
                                  ...
                                  Expanded(
                                    child: Text(
                                      'Driver sedang di: $_driverLocationText',
                                      style: TextStyle(
                                        fontSize: 13,
                                        fontWeight: FontWeight.w600,
